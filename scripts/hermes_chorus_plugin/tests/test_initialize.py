...
def test_initialize_uses_default_ring_when_cwd_unknown(isolated_env, monkeypatch):
    from plugins.memory.chorus import ChorusMemoryProvider

    (isolated_env / "chorus.json").write_text('{"default_ring": "core", "url": "http://localhost:3099"}')
    monkeypatch.setenv("CHORUS_API_KEY", "sk-test")
    ...
    with patch("plugins.memory.chorus.ChorusClient", return_value=fake_client):
        provider.initialize("sess-1")  # no cwd
